 ...
 * the linker rather than by good behaviour. scsi_write()/scsi_erase()
 * and their setup helpers are removed and .write/.erase are left out of
 * this KEEP'd ops struct, so no WRITE10/UNMAP path exists in the image
 * (blk_write()/blk_erase() then return -ENOSYS at runtime) and the
 * build gate can assert the symbols' absence.
 */
